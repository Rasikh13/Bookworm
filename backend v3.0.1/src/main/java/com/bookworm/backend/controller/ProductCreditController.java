...
public class ProductCreditController {
    ...
    @PutMapping("/{productCreditId}")
    @Operation(summary = "Edit an existing credit's type/value in place (Admin CMS)")
    public ResponseEntity<ApiResponse<ProductCreditResponse>> updateCredit(
            @PathVariable Long productId, @PathVariable Long productCreditId,
            @Valid @RequestBody ProductCreditRequest request) {
        return ResponseEntity.ok(ApiResponse.success(
                "Credit updated", productCreditService.updateCredit(productId, productCreditId, request)));
    }
