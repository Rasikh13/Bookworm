...
public interface UserShelfRepository extends JpaRepository<UserShelf, Long> {
    ...
    List<UserShelf> findByUserIdOrderByAcquiredAtDesc(Long userId);
    ...
    // Used by AcquisitionEligibilityServiceImpl - "is this RENT/LIBRARY grant still
    // live right now" (PURCHASE rows have expiresAt=null and are checked separately
    // via existsByUserIdAndProduct_ProductIdAndSource, since "after now" can never
    // match a null column).
    boolean existsByUserIdAndProduct_ProductIdAndSourceAndExpiresAtAfter(
            Long userId, Long productId, UserShelf.Source source, LocalDateTime now);
