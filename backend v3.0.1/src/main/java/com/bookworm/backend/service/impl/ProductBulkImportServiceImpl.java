...
public class ProductBulkImportServiceImpl implements ProductBulkImportService {
    ...
    private BulkImportRowResult importLegacyRow(Row row, Map<String, Integer> columnIndex,
        ...
        com.bookworm.backend.entity.Product.MediaType mediaType = switch (categoryName.toLowerCase()) {
            case "audiobooks", "audio books" -> com.bookworm.backend.entity.Product.MediaType.AUDIOBOOK;
            case "videos" -> com.bookworm.backend.entity.Product.MediaType.VIDEO_COURSE;
            default -> com.bookworm.backend.entity.Product.MediaType.BOOK;
        };
        ...
        // (see its validateRentFields javadoc) - it's what RoyaltyServiceImpl uses
        // to value a borrow (rentRate x borrowDays). This legacy export has no
        // per-day rental rate column at all, so derive a conservative one from the
        // purchase price rather than leaving every imported title unborrowable /
        // silently earning beneficiaries $0.00 per borrow.
        request.setRentRate(effectivePrice.multiply(new BigDecimal("0.05")).setScale(2, java.math.RoundingMode.HALF_UP));
        request.setIsAvailable(true);

        ProductResponse created = productService.create(request);

        // The real export carries an explicit "product_name_in_english" column
        // alongside the Devanagari/Marathi "prod name" that becomes the base
        // Product.title. That English title was previously read nowhere in this
        // importer and silently dropped - so no ProductTranslation row ever
        // existed for it, and the language selector had nothing to overlay when
        // a user picked English (see ProductServiceImpl.overlayTranslation,
        // which only ever shows base text when no translation row exists).
        // Capturing it here as an English ProductTranslation is what makes the
        // display-language dropdown and bilingual search actually work for
        // catalog data imported from this export.
        String englishTitle = cellValue(row, columnIndex, formatter, "product_name_in_english");
        if (StringUtils.hasText(englishTitle) && !englishTitle.trim().equalsIgnoreCase(prodName.trim())) {
            Language english = languageRepository.findByLanguageNameIgnoreCase("English")
                    .orElseGet(() -> languageRepository.save(Language.builder().languageName("English").build()));
            ProductTranslationRequest translationRequest = new ProductTranslationRequest();
            translationRequest.setLanguageId(english.getLanguageId());
            String trimmedEnglishTitle = englishTitle.trim();
            translationRequest.setTitle(
                    trimmedEnglishTitle.length() > 200 ? trimmedEnglishTitle.substring(0, 200) : trimmedEnglishTitle);
            productTranslationService.upsert(created.getProductId(), translationRequest);
        }
    ...
    private ProductResponse importRow(Row row, Map<String, Integer> columnIndex, DataFormatter formatter) {
        ...
        } else if (isLibraryEligible) {
            // ProductServiceImpl.validateRentFields now requires rentRate whenever
            // isLibraryEligible=true (RoyaltyServiceImpl values a library borrow as
            // rentRate x borrowDays - a null rate meant every borrow of a
            // library-only product silently earned beneficiaries $0.00, which was
            // the actual root cause of "library borrows aren't recording royalty").
            // A generic import sheet has no "rentrate" column at all for
            // non-rentable rows, so fall back to a conservative per-day rate
            // derived from price rather than rejecting the whole import row.
            BigDecimal optionalRate = cellDecimalOrNull(row, columnIndex, formatter, "rentrate");
            request.setRentRate(optionalRate != null ? optionalRate
                    : price.multiply(new BigDecimal("0.05")).setScale(2, java.math.RoundingMode.HALF_UP));
        }
        request.setIsAvailable(true);
