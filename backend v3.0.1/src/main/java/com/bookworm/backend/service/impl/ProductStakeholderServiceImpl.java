...
public class ProductStakeholderServiceImpl implements ProductStakeholderService {
    ...
    @Override
    @Transactional
    public ProductStakeholderResponse updateCredit(Long productId, Long productStakeholderId, ProductStakeholderRequest request) {
        ...
        if (!credit.getProduct().getProductId().equals(productId)) {
            throw new ResourceNotFoundException("ProductStakeholder", "product_stakeholder_id", productStakeholderId);
        }
        Stakeholder stakeholder = stakeholderRepository.findById(request.getStakeholderId())
                .orElseThrow(() -> new ResourceNotFoundException("Stakeholder", "stakeholder_id", request.getStakeholderId()));

        // Same duplicate-guard as addCredit, but excluding this row's own id -
        // otherwise "edit role text" on an unchanged stakeholder/role pair would
        // spuriously reject against itself.
        boolean duplicateOfAnotherRow = productStakeholderRepository
                .findByProduct_ProductId(productId).stream()
                .anyMatch(other -> !other.getProductStakeholderId().equals(productStakeholderId)
                        && other.getStakeholder().getStakeholderId().equals(request.getStakeholderId())
                        && other.getRole().equals(request.getRole()));
        if (duplicateOfAnotherRow) {
            throw new DuplicateResourceException(
                    "This stakeholder is already credited with this role on this product");
        }

        credit.setStakeholder(stakeholder);
        credit.setRole(request.getRole());
        return mapper.toResponse(productStakeholderRepository.save(credit));
    }
