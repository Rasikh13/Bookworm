...
class UserLibraryServiceImplTest {
    ...
    void setUp() {
        service = new UserLibraryServiceImpl(
                ...
                purchaseItemRepository, royaltyService, acquisitionEligibilityService, mapper);
    ...
    @Test
    void borrow_withNullRentRate_throwsRatherThanRecordingZeroRoyalty() {
        // UserLibraryServiceImpl now fails loudly instead of silently recording a
        // zero-value royalty row when a library-eligible product has no rentRate
        // configured (see the javadoc-style comment above the guard in borrow()).
        ...
        verify(royaltyService, never()).recordForProduct(any(), any(), any(), any());
    }
