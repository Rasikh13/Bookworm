...
public class UserLibraryServiceImpl implements UserLibraryService {
    ...
    public UserLibraryResponse borrow(Long userId, BorrowRequest request) {
        ...
        // rentRate is null for products that aren't independently rentable, in which
        // case the borrow has no attributable gross amount (mirrors RentServiceImpl's
        // own null-safety) and beneficiaries correctly accrue nothing for it.
        BigDecimal rentRate = product.getRentRate() != null ? product.getRentRate() : BigDecimal.ZERO;
        BigDecimal grossAmount = rentRate.multiply(BigDecimal.valueOf(request.getBorrowDays()));
